...
- **Safe hook path.** The canonical rules path is embedded in a shell command. `--install` therefore refuses a home/project path containing characters that remain active inside a quoted command (substitution, quote, escape, or line-break characters; on Windows, environment-expansion characters too). It exits 3 before creating the rules file or settings. Move the checkout, or wire the hook by hand with quoting appropriate to your shell.
...
- **Wrapper re-decision**: a leading wrapper (`env`, `sudo`, `timeout`, `nice`, `time`, …) or shell reserved word (`{`, `!`, `if`, `then`, `while`, `do`, …) runs or introduces the command after it, so that command's rules apply too. This stops `env aws …` from laundering a denied command through a broad `Bash(env:*)` allow, and stops `{ sudo …; }` from escaping a `sudo` deny by wearing a group opener.
