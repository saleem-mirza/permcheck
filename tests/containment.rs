...
fn wider_allow_does_not_override_overlapping_deny() {
    let rs = RuleSet::load_str(
        r#"{"allow":["Read(/**/passwd)"],"deny":["Read(/etc/**)"]}"#,
    )
    .unwrap();
    // Both rules match -> deny survives (allow is not a carve-out).
    assert_eq!(decide_payload(&rs, "Read", "/etc/passwd", None).tier, Tier::Deny);
    // Only the allow matches here -> allow.
    assert_eq!(decide_payload(&rs, "Read", "/home/u/passwd", None).tier, Tier::Allow);
}
...
fn exact_allow_carves_out_broad_deny() {
    ...
    assert_eq!(decide_payload(&rs, "Read", "/etc/passwd", None).tier, Tier::Allow);
    assert_eq!(decide_payload(&rs, "Read", "/etc/shadow", None).tier, Tier::Deny);
}
...
fn incomparable_overlap_resolves_to_deny() {
    let rs = RuleSet::load_str(
        r#"{"allow":["Read(/etc/*.conf)"],"deny":["Read(/etc/secret*)"]}"#,
    )
    .unwrap();
    assert_eq!(decide_payload(&rs, "Read", "/etc/secret.conf", None).tier, Tier::Deny);
    // Each rule still governs the region the other does not reach.
    assert_eq!(decide_payload(&rs, "Read", "/etc/app.conf", None).tier, Tier::Allow);
    assert_eq!(decide_payload(&rs, "Read", "/etc/secret.key", None).tier, Tier::Deny);
}
...
fn bash_narrow_allow_carves_out_broad_deny() {
    let rs =
        RuleSet::load_str(r#"{"allow":["Bash(aws * describe-*)"],"deny":["Bash(aws:*)"]}"#).unwrap();
    assert_eq!(decide_bash("aws ec2 describe-instances", &rs, None).tier, Tier::Allow);
    assert_eq!(decide_bash("aws s3 rm s3://b --recursive", &rs, None).tier, Tier::Deny);
}
...
fn narrow_allow_carves_out_bare_deny() {
    let rs = RuleSet::load_str(
        r#"{"allow":["WebFetch(https://api.example.com)"],"deny":["WebFetch"]}"#,
    )
    .unwrap();
    assert_eq!(
...
fn bare_allow_does_not_override_narrow_deny() {
    ...
    assert_eq!(decide_payload(&rs, "Read", "/etc/passwd", None).tier, Tier::Deny);
    assert_eq!(decide_payload(&rs, "Read", "/tmp/ok.txt", None).tier, Tier::Allow);
}
