...
|---|---|---|---|
| `deny_aws_describe` | `aws ec2 describe-instances` | deny (broad `aws:*`, no narrow allow) | ~1.5 µs |
| `deny_kubectl_get` | `kubectl get pods` | deny (broad `kubectl:*`) | ~1.5 µs |
| `ask_git_status` | `git status` | ask (no rule → `defaultMode` fall-back) | ~1.6 µs |
| `ask_git_push` | `git push origin main` | ask | ~1.8 µs |
| `deny_aws_terminate` | `aws ec2 terminate-instances` | deny | ~1.5 µs |
| `deny_kubectl_delete` | `kubectl delete pod x` | deny | ~1.5 µs |
| `deny_git_push_force` | `git push --force origin main` | deny (narrow deny > broad ask) | ~1.5 µs |
| `ask_unknown` | `some-tool --flag` | ask (`defaultMode` fall-back) | ~1.7 µs |
| `crosscheck_cat_env` | `cat .env` | deny (file-access cross-check) | ~1.7 µs |
| `crosscheck_redirect` | `echo hi > …/.ssh/authorized_keys` | deny (redirect cross-check) | ~4.1 µs |
| `wrapper_env_aws` | `env aws ec2 terminate-instances` | deny (wrapper re-decision) | ~2.7 µs |
| `compound_and` | `cd /tmp && ls -la` | (2 units) | ~2.9 µs |
...
|---|---|---|---|
| `read_allow_tmp` | `Read(/tmp/notes.txt)` | allow | ~2.8 µs |
| `read_deny_ssh` | `Read(/home/user/.ssh/id_rsa)` | deny | ~4.3 µs |
| `read_deny_env` | `Read(/home/user/.env)` | deny | ~3.1 µs |
